...
> **v6.41:** Combo UI #7+#8 CERRADO EN PROD (merge 2995e0b3). Fix spinner+wheel (#7) y sanitizado de paste numérico (#8) en los 3 forms LIVE de bobina (`AddCoilForm.tsx`, `PurchaseCoilFromXml.tsx`, `EditCoilModal.tsx`), campos Peso/Ancho/Espesor/Valor. Frontend-puro. Helper puro nuevo `src/core/coils/utils/numericInput.ts` (`sanitizeNumericPaste`/`NO_SPIN_CLASS`/`numericFieldHandlers`), RED (6/8 fallos por valor sobre stub passthrough) → GREEN (8/8) antes de implementar. `onWheel` hace blur del input (mata scroll accidental), `onPaste` intercepta y sanitiza (quita signo, comas de miles, espacios, pipes, colapsa a 1 punto decimal) en vez de dejar la sanitización nativa del navegador (que limpiaba el campo a `""` silenciosamente ante paste con formato). `onChange`/schema zod (`positive()`) intactos como red final. `npm run build` local verde antes de merge. Ticket #1 (moneda USD→SOLES en edición) reclasificado como WRITE backend dedicado — ver HANDOFF, el bug real es que `updateCoil` callable nunca persiste `currency`/`exchangeRate`.
