...
- **`page.test.tsx` — `renderiza filas mixtas` ROJO PREEXISTENTE (suite con 1 rojo de arrastre):** espera 2 filas (FFA1-101 + FFA1-102), renderiza 1 ("1 registros"). Error `Unable to find text: FFA1-102`, idéntico al baseline pre-#2 (confirmado por `git stash`, mismo error/DOM), independiente de si el mock apunta al helper viejo o nuevo — NO lo introdujeron #2 ni A, ambos solo tocaron el archivo. **Mecanismo AÚN NO diagnosticado** (síntoma: 1 fila donde se esperan 2; probable fixture/harness, no confirmado). Ya arrastra 2 frentes con el archivo tocado → hacerlo frente chico PRÓXIMO para devolver el suite a verde total. Al encararlo: EXIGIR el mecanismo real (por qué se renderiza 1 fila), no "es ajeno".
