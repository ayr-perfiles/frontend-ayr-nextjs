...
- **Frente A (buscador cola) EN CURSO — recon cerrado, sin implementar.** 131 cotizaciones en cola > cap fetch 100 → 31 invisibles. Plan: subir cap 100→500 (constante compartida con el banner) + matar N+1 (`getQuoteFulfillmentLogs` por-cotización → `getAllActiveFulfillmentLogs`+`bucketLogsBySourceId`, patrón ya vivo en /production/new) + activar `searchFields` en el `useTableData` de la cola (cliente + N° cotización, accessors por función). Client-side sobre set completo; **Algolia descartado a esta escala** (prod-only rompe paridad test, lag de índice en cola que se mueve, ahorro real = 31 lecturas). Frontend-puro. Tras A: Frente B (combobox /production/new), luego seguir con #4 según orden de prioridad.
