...
- ~~`revertProductionLog` drywall (`src/modules/drywall/services/productionService.ts`) — WRITE 7 drywall pendiente; además usa WAC-lookback, no costo congelado~~ ✅ **CORREGIDO (`[DOCS-STALE-SWEEP]` PASO 3, v6.74.0) — esta línea estaba mal en 2 ejes, no era una excepción real.** Ubicación: `revertProductionLog` es callable server-side, `functions/src/callables/drywallProduction.ts:206` (WRITE 7b, cerrado v6.23) — no vive en `productionService.ts` client-side. Mecanismo: usa **costo congelado**, no WAC-lookback — `calcRevertProductionFromCoil` (`functions/src/domain/drywallProduction.ts:130-156`) hace `ptValueBefore - log.stripCost`, restando el `stripCost` guardado en el propio `production_log` al momento de producir, nunca recalculando contra un WAC vivo. Ver `costeo-drywall.md` F-D6 — esa ficha también necesita el mismo ajuste, no verificada en esta tanda.
