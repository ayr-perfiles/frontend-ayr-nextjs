...
tocaron por este fix — en su momento seguían calificando para la cola de producción. ✅ **Retro CERRADO
...
> ⚠️ **BANNER (2026-08-27, v6.72.0 — alcance: SOLO el bullet de abajo; el resto de esta sección NO se re-verificó).** Ese bullet quedó PARCIALMENTE STALE. **"Duplicar Operación" ya NO está oculto ni roto:** es visible en `/admin/sales`, manda `?duplicateId=` correctamente, y se ejercitó en runtime en v6.72.0 (`V-000069` → `/admin/sales/new?duplicateId=V-000069&as=SALE`). Se arregló en #9-B.2a, NO en este frente. **"Editar Cotización" fue BORRADO en v6.80.0** (`[SALES-EDIT-404]`, opción (a) por decisión del dueño): el ítem del menú, la prop `onEdit` de `SalesTable` y su handler en `admin/sales/page.tsx` ya no existen. NO se re-apuntó a `/admin/quotations/[id]/edit`; la ruta unificada de documento sigue siendo frente propio del dueño. El call-site de "Duplicar Operación" usa hoy `router.push`, no `window.location` (`[NAV-HARD]`, v6.72.0).

**"Editar Cotización" y "Duplicar Operación" ocultos en `/admin/sales`** (`SalesTable.tsx`, `hidden:true`) — eran botones muertos, no arreglados, solo tapados: Editar apuntaba a `/admin/sales/[id]/edit`, ruta INEXISTENTE (404); Duplicar tenía un mismatch de query param (`?from=` que mandaba `page.tsx`, vs `?duplicateId=` que lee `new/page.tsx`) — abría un formulario vacío, no duplicaba nada. **Ambos resueltos desde entonces:** Duplicar se arregló y desocultó en #9-B.2a (v6.48.4/v6.48.5); Editar se BORRÓ en v6.80.0 (ver banner arriba).

**DEUDA VIVA — `status` no es atributo facetable en el índice Algolia `sales_index`.** Intentar filtrar server-side por status ahí (helper `buildAlgoliaStatusFilter`, escrito y testeado pero marcado `⚠️ NO USAR` en su docstring) **rompió la búsqueda de texto ENTERA en prod una vez** — con el filtro (aunque fuera un simple `status:COMPLETED`), Algolia respondía error de filtro inválido, el `catch` de `algoliaClient.ts` caía a `hits:[]`, y la búsqueda devolvía 0 resultados para CUALQUIER término, no solo para cotizaciones. Se revirtió el intento server-side y se resolvió filtrando client-side (ver arriba). **Encuadre corregido (v6.61.0, medido 2026-08-25):** los campos SÍ están en el record — 329/330 records de `sales_index` traen los 8 campos (`customerName`,`documentNumber`,`totalAmount`,`status`,`sellerId`,`timestamp`+`path`,`objectID`); el único sin campos es el record fantasma `SMOKE-EDIT-PROD-Q-NATIVA` (deuda aparte). Falta **SOLO declarar `attributesForFaceting`** en el dashboard (no re-indexar campos) — el save de facets de v6.61.0 cubrió únicamente `coils_index`, `sales_index` sigue igual (re-medido: `status:COMPLETED` → nbHits 0 con 191 reales). Cuando se declare, `buildAlgoliaStatusFilter` queda lista para reemplazar el filtro client-side sin tocar la lógica de whitelist (misma fuente `buildListStatusFilter`). Arquitectura de la búsqueda: `docs/03-arquitectura/busqueda-algolia.md`.
