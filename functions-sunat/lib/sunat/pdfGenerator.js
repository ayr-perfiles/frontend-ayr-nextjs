 ...
 */
function extractDigestValue(signedXml) {
 ...
 */
async function generateInvoicePdf(saleInfo, signedXml, sunatConfig) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const tipoCompTexto = saleInfo.documentType === "01"
                ? "FACTURA ELECTRÓNICA"
            ...
            const tipoDocCliente = saleInfo.customerDocument.length === 11 ? "6" : "1";
            ...
            const qrBuffer = await qrcode_1.default.toBuffer(qrString, {
                type: "png",
                width: 110,
            });
            ...
            doc.text("Representación impresa de la Factura Electrónica.", 160, currentY + 65);
            doc.end();
        }
        catch (error) {
            reject(error);
