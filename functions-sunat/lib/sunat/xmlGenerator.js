 ...
 */
function buildInvoiceXml(saleInfo, sunatConfig) {
    ...
    const invoiceLines = saleInfo.items.map((item, index) => {
        ...
        totalIgv += igvLinea;
        const itemDescription = item.name || item.description || "Producto de Planta";
        return {
    ...
    const despatchRefs = Array.isArray(saleInfo.guiasAsociadas) && saleInfo.guiasAsociadas.length > 0
        ? saleInfo.guiasAsociadas.map((guiaId) => ({
            "cbc:ID": guiaId,
    ...
    const doc = (0, xmlbuilder2_1.create)({ version: "1.0", encoding: "utf-8" }, xmlObj);
