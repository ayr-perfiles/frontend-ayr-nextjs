/**
 * Copia server-side (dominio 100% puro, sin `firebase-admin`) del builder canónico v6.28
 * `src/core/sales/domain/saleDocBuilder.ts`.
 *
 * PRINCIPIO (heredado del original, no negociable): este builder normaliza FORMA (shape
 * canónico de sales/quotations, businessLines derivado, flags, campos obligatorios),
 * NUNCA VALORES. Si el caller ya trae totales/profit con signo correcto (ej. multiplier
 * de Nota de Crédito aplicado), el builder los RESPETA tal cual — NO los recalcula desde
 * items. Solo calcula desde items cuando el caller no los trae (caso POS).
 * Por eso el chequeo es `!== undefined`, no truthy: 0 es un total legítimo.
 *
 * PROHIBIDO el spread dentro del builder — era la causa raíz que v6.28 vino a matar
 * (heredaba lo que el caller le pasara). Los campos se enumeran uno a uno, a propósito.
 *
 * Duplicación: patrón sancionado del repo (ADR "Dominio puro" + parity test), ver la nota
 * de los 4 bloqueos en `../catalog/classifyLine.ts`. Mantener en sync a mano;
 * `__tests__/buildQuotationDoc.parity.test.ts` es la única red de seguridad.
 *
 * ⚠️ PARA QUIEN LO CONSUMA DESDE UN CALLABLE DE EDICIÓN:
 *   - La salida es un doc de CREACIÓN (19 claves), no un doc completo. `buildQuotationDoc`
 *     fuerza `status:'QUOTATION'` y `productionStatus:'PENDING'`, y pisa `timestamp` con el
 *     `injectedTimestamp`. Hacer `set()` de esto sobre un doc existente CLOBBEA
 *     `productionStatus` (CONFIRMED -> PENDING, o sea que sale de la cola de producción),
 *     `timestamp`, `isFulfilled`, `confirmedBy`, `confirmedForProductionAt`,
 *     `convertedToId`, `costSyncedAt` y `annulledSaleRef`.
 *   - El override de totales (`input.totalX !== undefined`) existe para el importador/NC.
 *     Un callable de edición NO debe reenviar totales que vengan del cliente: `totalAmount`
 *     está protegido en `firestore.rules` justamente por ser el snapshot financiero.
 *   - Este módulo NO hace I/O: recibe los items con su `baseCost` ya resuelto. El recompute
 *     contra el WAC vivo es responsabilidad del callable, que sí puede leer stock.
 */
import { classifyLine } from "../catalog/classifyLine";
...
export interface InputSaleDoc {
  ...
  paymentStatus?: string;
  sellerId?: string;
  /** Semilla de flags del caller (importador). Se une con las flags derivadas por ítem. */
  allFlags?: string[];
  timestamp?: unknown;
}

export function buildSaleDoc(input: InputSaleDoc, injectedTimestamp: unknown): CanonicalSaleDoc {
  const items: CanonicalSaleItem[] = [];
  const businessLinesSet = new Set<string>();
  const skusSet = new Set<string>();
  const allFlagsSet = new Set<string>();

  for (const f of input.allFlags || []) {
    allFlagsSet.add(f);
  }
  ...
  for (const rawItem of input.items || []) {
    ...
    // Asimetría deliberada, copiada del original: `weightSnapshot` se emite por TRUTHY,
    // los otros dos por `!== undefined` (un `piecesCount: 0` SÍ se emite).
...
export function buildQuotationDoc(input: InputSaleDoc, injectedTimestamp: unknown): CanonicalQuotationDoc {
