...
export function BulkUploadCoils() {
  ...
  const handleUploadToFirebase = async () => {
    ...
    if (!defaultFinish) {
      toast.error("Por favor, selecciona un acabado por defecto.");
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      for (const coil of parsedCoils) {
        ...
        const coilData = {
          ...
          finish: defaultFinish, // Aplicar acabado por defecto
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative shrink-0">
            <Layers className="absolute left-3 top-3 text-purple-400" size={18} />
            <select
              value={defaultFinish}
              onChange={(e) => setDefaultFinish(e.target.value)}
              className="pl-10 pr-4 py-2 bg-purple-50 border border-purple-200 text-purple-700 rounded-xl font-bold outline-none focus:ring-2 focus:ring-purple-200 transition appearance-none"
            >
