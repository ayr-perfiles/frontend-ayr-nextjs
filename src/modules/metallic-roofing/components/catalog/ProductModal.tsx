...
export function buildFinishesToRender(finishes: CoilFinish[], formFinishes: string[]): Array<CoilFinish & { active: boolean }> {
  const metallicFinishIds = finishes.filter(f => f.active && f.lines.includes("metallic-roofing")).map(f => f.id);
  const activeIds = new Set(metallicFinishIds);
  const finishesToRender = finishes.filter(f => activeIds.has(f.id));
  
  for (const fId of formFinishes) {
    if (!activeIds.has(fId)) {
      const existing = finishes.find(f => f.id === fId);
      finishesToRender.push({
        id: fId,
        label: existing ? existing.label : fId,
        active: false,
  ...
  return finishesToRender;
}

export function shouldShowLengthField(family: string | undefined): boolean {
  return family === "PLANCHA";
}

export function buildProductInput(form: AddMetallicProductFormState, isCreate: boolean = false): MetallicProductInput {
  const input: Partial<MetallicProductInput> = {
  ...
  if (form.width) input.width = parseFloat(form.width);
  if (form.family === "COBERTURA") {
    (input as any).length = deleteField();
  } else if (form.length) {
    input.length = parseFloat(form.length);
  }
  if (isCreate && form.sku) input.sku = form.sku;
  if (form.displayName) input.displayName = form.displayName;
  if (form.widthMm) {
    input.widthMm = parseFloat(form.widthMm);
    input.metaSource = "manual";
  }
  return input as MetallicProductInput;
}
...
export default function ProductModal({ mode, product, onClose, onSuccess }: ProductModalProps) {
  ...
  const finishesToRender = buildFinishesToRender(finishes, form.finishes || []);
  ...
  // density factor logic
  const finishesWithoutDensity = (form.finishes || []).filter(fId => {
    const f = finishes.find(x => x.id === fId);
    return !f || f.densityFactor == null || f.densityFactor <= 0;
  });
  ...
  async function handleSubmit() {
    ...
    const input = buildProductInput(form, isCreate);
        ...
        <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                  ...
                  {finishesToRender.map((f) => {
                    ...
                    return (
                      <button
                        ...
                        title={!f.active ? "Acabado obsoleto/inactivo" : ""}
                        className={`px-3 py-2.5 rounded-xl text-[10px] font-bold transition border text-left flex flex-col justify-between h-full relative ${
                          isSelected
                            ? !f.active 
                              ? "bg-amber-800 border-amber-900 text-amber-50 shadow-md shadow-amber-300/50 ring-2 ring-amber-400 ring-offset-1"
                        ...
                        } ${!f.active && !isSelected ? "opacity-50" : ""}`}
                      >
                        <span className="leading-tight flex items-center justify-between w-full">
                          {f.label}
                          {isSelected && <CheckCircle2 className={`w-3.5 h-3.5 ${!f.active ? "text-amber-300" : "text-zinc-300"}`} />}
                        </span>
                        {!f.active && isSelected && (
                          <span className="text-[8px] text-amber-300 mt-1 uppercase">Obsoleto</span>
                        )}
                      </button>
                    );
                  })}
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              {shouldShowLengthField(form.family) && (
                <div>
                  <label className="text-[10px] font-bold text-gray-400 mb-1 block">
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {finishesWithoutDensity.length > 0 ? (
                      <div className="w-full p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-2">
                        <AlertTriangle size={14} className="text-amber-500 shrink-0" />
                        <p className="text-[10px] font-bold text-amber-700">
                          Acabado(s) &quot;{finishesWithoutDensity.join(", ")}&quot; sin densidad configurada — producción bloqueará.
                        </p>
                      </div>
                    ) : (
                      <div className="w-full p-3 bg-blue-50 border border-blue-200 rounded-xl font-black text-blue-800 text-sm flex flex-col gap-1">
                        {(form.finishes || []).map(fId => {
                          const f = finishes.find(x => x.id === fId);
                          return (
                            <div key={fId} className="flex justify-between items-center border-b border-blue-100/50 last:border-0 pb-1 last:pb-0">
                              <span>{f?.densityFactor}</span>
                        ...
                        {(!form.finishes || form.finishes.length === 0) && (
                          <span className="text-[10px] text-blue-400 font-normal">Ninguno</span>
                        )}
                      </div>
                    )}
                  </div>
