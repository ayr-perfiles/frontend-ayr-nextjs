...
export default function MovementsHistoryModal({ item, onClose }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error("[MovementsHistoryModal metallic-roofing]", err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-5 bg-zinc-700 text-white flex justify-between items-start shrink-0">
          <div>
            <h2 className="text-lg font-black">Historial de Movimientos</h2>
            <p className="text-zinc-300 text-xs font-bold mt-0.5">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="relative">
              <select
                ...
                className="appearance-none pl-3 pr-8 py-2 bg-gray-50 border border-gray-200 rounded-xl font-bold text-xs outline-none focus:border-zinc-400 cursor-pointer"
              >
            ...
            <input
              ...
              className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl font-medium text-xs outline-none focus:border-zinc-400"
            />
                      ...
                      <td className="px-5 py-3.5">
                        {saleRef && (
                          <a
                            ...
                            className="inline-flex items-center gap-1 text-zinc-600 hover:text-zinc-700 text-xs font-bold transition"
