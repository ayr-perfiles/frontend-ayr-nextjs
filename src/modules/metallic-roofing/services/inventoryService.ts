...
import type { MetallicProduct, MetallicStock, MetallicStockMovement } from '../types';
...
export interface InventoryItem extends MetallicStock {
  product: MetallicProduct | null;
}
...
export interface StockMovement extends MetallicStockMovement {
  id: string;
}
...
export async function fetchInventory(filters: InventoryFilters): Promise<InventoryItem[]> {
  ...
  const stockMap = new Map<string, MetallicStock>();
  for (const d of stockSnap.docs) {
    stockMap.set(d.id, { sku: d.id, ...d.data() } as MetallicStock);
  }
...
export async function getStock(sku: string): Promise<MetallicStock | null> {
  const snap = await getDoc(doc(db, STOCK_COL, sku));
  if (!snap.exists()) return null;
  return { sku: snap.id, ...snap.data() } as MetallicStock;
}
