...
const STOCK_COL = 'metallic_roofing_stock';
const MOVEMENTS_COL = 'metallic_roofing_stock_movements';
const CATALOG_COL = 'metallic_roofing_catalog';
...
export interface AdjustStockInput {
  ...
  /** Siempre positivo para ENTRY/EXIT; para ADJUSTMENT es el valor absoluto nuevo */
  quantity: number;
...
export async function adjustStock(input: AdjustStockInput): Promise<StockMovement> {
  ...
  if (!input.reason.trim()) throw new Error('El motivo del ajuste es obligatorio.');
  ...
  return runTransaction(db, async (tx) => {
    // ── ALL READS FIRST ──
    const [stockSnap, catalogSnap] = await Promise.all([tx.get(stockRef), tx.get(catalogRef)]);
    ...
    const delta =
      input.type === 'ENTRY'
        ...
        : input.type === 'EXIT'
          ? -input.quantity
          : input.quantity - currentQty; // ADJUSTMENT = valor absoluto

    const newQty = currentQty + delta;

    // Costo promedio ponderado — solo en ENTRY con costo positivo
    ...
    const auditDetails = `[${input.type}] SKU ${input.sku} | Δ${delta > 0 ? '+' : ''}${delta} | Stock: ${currentQty} → ${newQty} | Motivo: ${input.reason}`;
