...
describe("calculateExpectedPiecesByDensity", () => {
  describe("cálculos correctos con datos válidos", () => {
    ...
    it("debe calcular para parante P64 (fleje ancho)", () => {
      ...
      const result = calculateExpectedPiecesByDensity(1000, 149, 0.45, 3.0, 0.00785);
    ...
    it("debe manejar largo de pieza no estándar (2.44m)", () => {
      ...
      const result = calculateExpectedPiecesByDensity(1000, 121, 0.45, 2.44, 0.00785);
  ...
  describe("validaciones de entrada", () => {
    it("debe retornar 0 si el peso es 0", () => {
      const result = calculateExpectedPiecesByDensity(0, 121, 0.45, 3.0, 0.00785);
      expect(result).toBe(0);
    });

    it("debe retornar 0 si el espesor es 0 (división por cero)", () => {
      const result = calculateExpectedPiecesByDensity(1000, 121, 0, 3.0, 0.00785);
      expect(result).toBe(0);
    });

    it("debe retornar 0 si el ancho es 0", () => {
      const result = calculateExpectedPiecesByDensity(1000, 0, 0.45, 3.0, 0.00785);
      expect(result).toBe(0);
    });

    it("debe retornar 0 si el largo de pieza es 0", () => {
      const result = calculateExpectedPiecesByDensity(1000, 121, 0.45, 0, 0.00785);
      expect(result).toBe(0);
    });

    it("debe retornar 0 si hay valores negativos", () => {
      expect(calculateExpectedPiecesByDensity(1000, -121, 0.45, 3.0, 0.00785)).toBe(0);
      expect(calculateExpectedPiecesByDensity(1000, 121, -0.45, 3.0, 0.00785)).toBe(0);
      expect(calculateExpectedPiecesByDensity(1000, 121, 0.45, -3.0, 0.00785)).toBe(0);
      expect(calculateExpectedPiecesByDensity(-100, 121, 0.45, 3.0, 0.00785)).toBe(0);
    });
  });

  describe("casos extremos", () => {
    it("debe manejar bobinas muy pesadas (5000kg)", () => {
      const result = calculateExpectedPiecesByDensity(5000, 121, 0.45, 3.0, 0.00785);
    ...
    it("debe manejar flejes muy angostos (60mm)", () => {
      const result = calculateExpectedPiecesByDensity(1000, 60, 0.45, 3.0, 0.00785);
      // Fleje más angosto = más metros = más piezas
      expect(result).toBeGreaterThan(780);
    });

    it("debe manejar espesor muy delgado (0.30mm)", () => {
      const result = calculateExpectedPiecesByDensity(1000, 121, 0.3, 3.0, 0.00785);
    ...
    it("debe redondear hacia abajo (piezas enteras)", () => {
      ...
      const result = calculateExpectedPiecesByDensity(1000, 121, 0.45, 3.001, 0.00785);
  ...
  describe("validación contra límite físico (5% tolerancia)", () => {
    it("debe respetar el límite del 5% en validación backend", () => {
      const expected = calculateExpectedPiecesByDensity(1000, 121, 0.45, 3.0, 0.00785); // 779
